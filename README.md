...
七个 provider 的 Account Hub 面板都提供「**模型列表**」按钮，可逐个开关模型以控制其
...
`dsh-account-hub`（设置页显示「账号中心」）。**只有品牌层改名**，代码标识符与
存储键一律未动，因此迁移不丢数据。
...
> 保持原样** —— 变的只有包名与界面文案，所以重装后账号池、登录状态与模型列表设置
> 直接续用，无需重新登录。
>
...
- **UI 拖拽已接**（客户端 `plugin-src/client/account-order.js` + `account-hub.js`）：
  按住卡片拖动即可改顺序，源卡片半透明、目标卡片按指针落在**上半/下半区**显示
  插入线（上=插其前、下=插其后）。松手**本地立即生效**（乐观更新）再发
  `account.reorder`；宿主拒绝时**回滚原顺序**并提示，不会停在「看起来已生效」
  的状态。拖回原位、拖到自己身上、只有一个账号时都**不发请求**（一个账号时
  干脆不启用拖拽）。顺序即上面的候选优先级 —— **与「消耗顺序」三档的关系是：
  三档都消费这个顺序**，档位决定「在这个顺序上怎么取号」（取第一个 / 轮转 /
  按余额优先），拖拽决定「顺序本身长什么样」。
  ⚠️ 落点判定必须区分 before/after（只支持「插到之前」会让「往下拖一格」变成
  空操作），且**移除源元素后目标下标会前移一位**、不能复用原下标 ——
  两条都有纯函数单测（`tests/unit/account-order.spec.ts`）与渲染级事件测试
  （`tests/unit/account-order-panel.spec.ts`）钉死。

### 账号消耗顺序 / 切换粒度（2026-09-24）

账号中心每个 provider 面板**顶部**（账号卡片列表之前）并排两个下拉，per-provider 各存各的。
界面上**只有这两个下拉**：无外框、无可见标签、无内联解释 —— 设置名与各档含义
（即下表内容）合并在下拉的**悬停提示**里（`title`），可读名在 `aria-label`。

| 消耗顺序 | 含义 |
|---|---|
| **顺序** | 总是取排序里的第一个可用账号 |
| **遍历**（默认） | 每次请求轮转下一个账号，用完一轮再从头开始（游标持久化，重启后接着轮） |
...
> ⚠️ **默认档是「遍历」**（`round-robin`）。从没动过这项配置的用户，升级后选号行为会
> 由「固定用排序第一个账号」变为「逐请求轮转」—— 这是刻意的默认（多账号积分被均匀
> 消耗）。想回到老行为就在界面上选「顺序」，或把配置写成 `sequential`。
> 该默认值的翻转**不涉及文档形状**，故不提升 `ACCOUNT_HUB_SCHEMA_VERSION`、
> 也不需要任何数据迁移（旧文档没有 `consumption` 条目，读出来就是新默认值）。
...
- **余额缓存在宿主内存里**（TTL 5 小时，启动后与每 4 小时刷新 —— 间隔严格小于 TTL，
  留 1 小时余量防刷新晚一拍出现空窗），**且只为配了「最高优先」
...
> 怎么取号** —— 三档都消费同一个顺序：`sequential` 取排序里的第一个可用账号、
> `round-robin` 沿这个顺序轮转、`highest-balance` 在这个顺序的可用账号里挑余额
> 最高的（并列时靠前的优先）。所以「把某账号拖到第一位」在任何档位下都有意义。
...
Account Hub 的 **Codearts 面板**点「登录账号」时，RPC **不再**在请求内等待浏览器
...
- **思考档位在面板条目上配，一个条目一个档位**：条目配了档位就用条目的（自动模型 =
  provider + model + 档位的打包语义）；没配就落**目标模型自己的默认档**。
  ⚠️ **DSH 会话侧没有档位下拉**（聚合模型刻意不声明思考档位）—— 会话里那个下拉对
  自动路由站不住：条目配了档位时选了也不生效；没配时它只反映**队首**的档位表，而降级
  换队首后就会失配。**想要不同档位就建多个自动模型**（同一 provider/model、不同条目档位）。
...
- **上下文窗口声明值取「最大档」**：`min(maxInputTokens, contextWindow.supportedLengths`
  ...
  **`defaultLength` 是官方客户端的 UI 默认档、不是服务端硬限** —— 2026-09-21 钳制二分
  ...
  无关），但 Account Hub
  的「模型列表」里**可以选择更小的档位**（`supportedLengths` 的那些值，见
  [上下文窗口档位选择](#上下文窗口档位选择)）。
...
- 凭据 ref：
  ...
  - 多账号：`BUDDY_ACCOUNT_<UUID_SHORT>`，由 Account Hub 设置页「登录账号」
...
- 面板提供账号列表、登录账号（浏览器登录入池）、启用/停用、删除，以及供应商级的
  「模型列表」（模型开关）与「重测所有 / 清除限额」限流标记操作
  （`account.retestAll` / `account.resetAll`），行为与 Buddy CN 面板一致，但
...
元数据的五个 provider，而「模型列表」七个 provider 一个都不能少）。省略时即上述降级，
这是 headless / 测试的既定行为。
...
「模型列表」弹窗的模型集合**不只是**适配器播报的那份目录，而是
...
> 实例均已就位，Trae CN 面板可以登录账号、刷新凭据与重测限流标记。
...
3. 否则调领取端点（`POST …/checkin_credits/claim`，同样 body
   `{"req_source":1}`），失败码按**三档**分流：`9095` → `already-claimed`
   （设备今日已签）、`9074` → `unavailable`（暂不可签，等 4h sweep 重试）、
   其余 → `failed`；命中**可重试码**时先做请求内退避重试（见下）；
...
> 缺了直接回 `code:9004`。
>
> ⚠️ **T9 第三次修正（2026-09-20）的归因在第五次定案中回到了原点**：T9 原结论
> 「status / claim **都不校验设备号形态**」（16 位十进制号 / `BoundDeviceID` /
> 空串全回 `code:0`）**观测仍然成立** —— 服务端确实不校验设备号的**形态**。它推出的
> 「不校验形态 ≠ 不校验**设备**」**最终是对的**（第 4 次定性一度把这条又否掉，
> 但那次已被 2026-09-23 的单变量矩阵作废）：服务端认的是「**这个号有没有进过
> 黑名单**」，而**不是**「它是不是登录时那个」。设备头仍然**必须带**（缺了回
> `9004`），且遇 `9074` 要**换一个同形态的新号重试** —— 详见下方「`9074` 的定性」。
>
...
#### `9074` 的定性（2026-09-23 **第五次**修正）与头集对齐
...
| 第 2 次 | 活动级当日容量/名额限制或账号侧风控 | **误读**：当时见官方客户端同期签成功，就以为「不是名额」 |
| 第 3 次 | **设备身份**：服务端按 `x-device-id` 记设备维度签到状态，我们发的 `BoundDeviceID` 不被活动系统认可 | 真机单变量矩阵 —— 账号级已签时**任意设备号**都回 `code:0` |
| 第 4 次 | **名额/风控类拒绝**：此刻不受理这次领取，**与设备号取值无关** | 真机单变量矩阵 —— 换**全新**设备号 11 秒后首次 claim 即 `code:0` |
| **第 5 次（现行）** | **设备号被服务端拉黑**：该号在**未产出奖励**的 claim 中出现过 | 真机单变量矩阵（2026-09-23，下述） |

⚠️ **第 3 次与第 4 次的错处正好相反，而两次都是同一个矩阵纠出来的**：服务端
**确实在认设备**（第 4 次错在否定了这一点），但判据**不是**「这个号是不是登录时
那个」（第 3 次错在把它当成了判据），而是「**这个号有没有进过黑名单**」。

**真机单变量矩阵**（`POST …/checkin_credits/claim`，2026-09-23 **第五次定案**）：

| 账号级 `checked_in` | 设备级 `did_checked_in` | 设备号状态 | 响应 |
|---|---|---|---|
| 已签 | 任意 | 任意 | `code:0`（成功 —— **任意设备号**都成功） |
| 未签 | 已签 | 任意 | `code:9095`「当前设备今日已经签到」 |
| 未签 | 未签 | **干净**（服务端没见过） | `code:0`（**首次 claim 即成功**） |
| 未签 | 未签 | **被拉黑** | `code:9074`「当前参与用户太多，请稍后再试」 |

**最后一行的对照就是第 5 次定性的证据**：**同一账号、同一 token、11 秒间隔**，
换一个**全新 16 位设备号**首次 claim 直接 `code:0`；换回旧号立刻又是 `9074`
（独立复现）。故 `9074` 的语义是**「这个设备号在未产出奖励的 claim 里出现过，
被服务端拉黑了」**。

这一行**同时推翻了第 3 次与第 4 次定性**：

- 第 3 次说「服务端只认登录时那台设备」—— **错**。账号级已签时任意设备号都回
  `code:0`，服务端并没有「只认登录号」这回事；旧证据把 `did_checked_in` 这个
  **设备级字段**的回退读成了原因，它其实是结果。
- 第 4 次说「名额/风控类拒绝，与设备号取值无关」—— **也错**。真与服务端无关的话，
  11 秒后的全新号不可能立刻成功。

⚠️ **已被矩阵排除的候选**（不要再回头查）：传输头 / 会话 / `req_source` / UA
**都不是**条件。官方客户端用**机器级稳定 AHA 号**；部分账号的
`checkin_device_id` 是 exchange 绑定的稳定号（**重登不换号**），故重登治不了它
—— 这正是「河童重登无效」之谜。

> ⚠️ **设备头仍然必须带**（缺了回 `code:9004`）：`x-device-id` 取自凭据的
> `checkin_device_id`（登录时生成并上报的 16 位号），另有 `x-device-type` /
> `x-os-version` / `x-app-version`。**出站协议值零变更** —— 换号路径只改
> `x-device-id` 的**取值**（换成一个同形态的新号），头集 / body / 端点全部未动。

**9074 的处置：换新设备号重试一次**（用户 **2026-09-23 拍板**）：

```text
claim → 9074  ⇒  rotateTraeCnCheckinDeviceId()（登录时同一个生成器，新 16 位号）
                 → 立刻重发 claim（1 次，硬编码不循环）
                 → 写回凭据（两种结果都写：checkin_device_id + device_id_source）
                    ├─ 重试成功      → claimed
                    ├─ 重试仍 9074   → unavailable（不写 checkins，4h sweep 下轮用新号再试）
                    └─ 重试其它失败码 → failed（按最后一次现场报）
```

**为什么是换号而不是退避**：退避再久也不会让一个被拉黑的号变得可用。故 `9074`
**不**进 `TRAE_CN_CLAIM_RETRY_CODES`（那张表是「等几秒再问同一个请求」），
它有自己的一次换号重试，次数**硬编码 1**（`TRAE_CN_ROTATE_DEVICE_RETRY_LIMIT`）
—— 一个刚生成的全新号不该再被拉黑，若仍回 9074 说明另有原因，继续摇号没有依据。

**为什么两种结果都要写回**：写回决定的是**下一轮 sweep 的起点**。漏掉失败分支的
写回，每轮都会先拿旧号撞一次注定 9074 的请求 —— 功能上仍会成功（换号后那一次），
但每轮白烧一次往返。

**凭据写回机制**（不绕开 `ctx.credentials`）：
`TraeCnCreditsOptions.persistCredential` → `CreditsEndpointDeps.persistCredential(refName, credential)`
（`collectClaimResults` 按**当前账号**的 `entry.credentialRef` 闭包出 `refName`）
→ `ctx.credentials.set(credentialRef(refName), serializeTraeCnCredential(…))`。
**只在真的换过号时调用**（首发成功 / 非 9074 失败都不调）。写回失败**只记
`onDebug`、不炸签到主流程**。三条入口（单账号 `checkin.perform` / 一键
`credits.claimAll` / 4h `checkin.sweep`）共用 `runCreditsClaim` 的同一分支 ⇒
一处接线全覆盖，**无旁路**。

**四档处置**（claim 返回失败码时的分流，`src/trae-cn-credits.ts`）：

| 业务码 | 归一 outcome | 语义与后果 |
|---|---|---|
| `code:0` | `claimed` | 领取成功（随后补查 status 取积分） |
| `9095` | `already-claimed` | 账号未签但**这台设备今天已签** ⇒ 这份奖励已到手，写今日状态 |
| `9074` | **先换号重试一次**，结果按重试自己的码分流 | 重试成功 → `claimed`；仍 9074 → **`unavailable`**（**不写**今日状态，4h sweep 下轮用新号重试） |
| 其余（`1001` / `9004` …） | `failed` | 需要用户行动（重新登录 / 校准设备头） |

`unavailable` 与 `failed` **刻意分开**：`failed` 是「你需要做点什么」，
`unavailable` 是「什么都不用做，等自动重试」。把它报成失败会让用户去排查凭据
或设备 —— 而那两件事都治不了 `9074`。

**用户文案**（`src/trae-cn-credits.ts` 的 `describeFailureCode`；前端
`formatClaimFailureLine` 仍会在末尾统一追加 `（code 9074）`，故本函数**不**重复写 code）：

> 当前参与用户太多，请稍后再试（服务端此刻暂不可签，稍后自动重试）

（该文案只在**换号后仍是 9074** 时出现 —— 首发 9074 已经换过号了。）

⚠️ 旧文案「若本账号是旧版凭据登录的，请重新登录一次以登记设备身份」**已删除** ——
它指向的动作**治不了** `9074`（真因是设备号在黑名单里，不是「没登记设备」）。
错误指引比没有指引更坏：用户会照着做，做完仍然失败。

> ⚠️ **本改动可整体 revert**（用户约定：明天实测若无效即回退）。改动落点、
> 需要一并还原的测试断言、以及「出站协议值零变更」的边界，全部记在
> `docs/agents/providers-trae-cn.md` 的「`9074` 换号重试：用户拍板记录」。
...
**claim 段只对真正的瞬时软限流做有界退避重试**（`9074` 已于 2026-09-23 **移出**
这张表：它的重试是**换设备号那一次**，见上文；退避再久也不会让一个被拉黑的号
变得可用）：

| 项 | 值 |
|---|---|
| 可重试码 | `4007` `3004`（`TRAE_CN_CLAIM_RETRY_CODES`） |
...
| **绝不重试** | `9004`（设备头）、`1001`（凭据失效）、`9095`（今日已签）—— 确定性结果 |
| `9074` 的重试 | **换号重试一次**（`TRAE_CN_ROTATE_DEVICE_RETRY_LIMIT`，硬编码、不循环、不退避）；仍是 9074 ⇒ `unavailable`、不写今日状态 ⇒ 宿主 4h sweep 下轮用**新号**再试 |
| 判据来源 | 本地清单 **必须**是 `src/trae-cn-errors.ts` 的 `TRAE_CN_BACKOFF_CODES` 的**真子集**（上游移除某码即自动停重试） |

`3003`（`MODEL_FAIL`）虽然在共享退避表里，但它是 **chat 通道**的基础设施码，
签到端点没有对应观测，**刻意不放进** `TRAE_CN_CLAIM_RETRY_CODES`。

⚠️ **`9074` 仍在 chat 侧的共享退避表里**（chat 动作仍是「退避、不换号」，一行未动）：
两条协议线刻意分开，故签到侧的收窄只动那张**更窄的清单**。本模块的
`TRAE_CN_CLAIM_RETRY_CODES` 因此必须是**独立的一张表**，而不是直接引用共享表 ——
两处写成一份，「签到不再重试 9074」就无从表达。
...
「这个模型限流 N 分钟，等一会儿自动解除」，而 `9074` 是**设备号状态**（该号在
失败的 claim 里出现过、被服务端拉黑）—— 它与具体模型无关，解除方式也不是
「等一会儿」而是**换一个号**（本插件已自动做，见上文），记一个「该模型限流
N 分钟」的徽章是虚假信息。

`9074` 的用户文案因此只说**可执行的事实**（`src/trae-cn-credits.ts` 的
...
**「暂不可签」（`unavailable`）另成一段**，不混进失败明细列表：
摘要行写作「N 个暂不可签」，明细每行同样是「账号：服务端原文（code N）」
（`message` 缺失时回退「服务端此刻暂不可签」，**不是**「领取失败」）。
它的色调**仍是 `ok`** —— `failed` 为 0 就不切 warn。理由与上文同源：
这一档不需要用户做任何事，混进失败会让用户去排查并不存在的问题。
（目前唯一的生产者是 Trae CN 的 `9074`。）

**失败行末尾还会追加 `· logid <值>`**（当服务端给了时）——见下方
「失败诊断的 logid 透传」。「暂不可签」行同样透传 logid。
...
- **登录是两段式非阻塞的**（2026-09 起）：Account Hub 点「登录账号」时，
...
- 凭据 ref：
  ...
  - 多账号：`LOBSTERAI_ACCOUNT_<UUID_SHORT>`，由 Account Hub「登录账号」生成。
...
- 模型列表：远端 `GET /api/models/available` 优先（它是权威来源），
  ...
  ⚠️ **模型列表端点必须带 `X-LobsterAI-Client-Capabilities` 头**（走
  `lobsteraiModelsHeaders`，`Accept` 仍为 JSON），否则上游按能力过滤、**只回部分
  模型**（实测 25 个且缺 `kimi-k3`；带上 `kimi-k3-agentic-v1` 才回 26 个）。
  两个坑**各自独立、叠加生效**，任一个都会让远端已上线的模型在面板里看不到。
...
> 形态要求来自**登录握手**。它与签到头的 `x-device-id`（取自凭据的
> `checkin_device_id`）是**两个位置** —— 详见「Trae CN provider」
> 章节的「设备号在本项目里是两个位置」。
> 早先把「形态不符」的后果记成「会触发 9074 风控」是**归因错误**：`9074` 与
> 设备号**形态**无关（它对任何形态的干净号都放行）。现行定性是**设备号被拉黑**
> —— 判据是「这个号有没有进过黑名单」，与形态无关，见「积分领取」章节的
> `9074` 小节。
...
> ⚠️ **「不伪造设备身份」的唯一例外（2026-09-23 用户拍板）**：这条禁令针对的是
> **「拿 `machine_id` 之类别的字段折算出一个看起来合法的号来掩盖缺失」**。签到
> 遇 `9074` 时**换一个全新 16 位设备号重试一次**（`rotateTraeCnCheckinDeviceId`，
> 用登录时**同一个生成器**）**不属此列** —— 它换的是一个**真实注册形态的随机
> 设备号**（服务端侧表现为 `did_checked_in` 归 false 的**新设备**），不是把没有
> 的说成有。真机矩阵已定案 `9074` = 该设备号被服务端拉黑，故换号是唯一有效动作。
> **完整实验证据、策略与 revert 约定见
> `docs/agents/providers-trae-cn.md` 的「`9074` 换号重试：用户拍板记录」**。
...
| `checkin_device_id` | **签到头 `x-device-id` 的来源**。两种来历：① 登录时生成并上报的 16 位设备号（= 登录 URL 的 `device_id`）—— 主路径；② **签到遇 `9074` 后轮换生成的新 16 位号** —— 由 `device_id_source` 的 `'rotated-after-9074'` 标出（两种来历**同形态**，同一个生成器）。旧凭据缺失时签到侧如实降级为 `device_id` |
| `device_id_source` | `device_id` 的来源标记（诊断用）。⚠️ 2026-09-23 起**兼述 `checkin_device_id` 的来历**：`'exchange-bound-device-id'`（登录路径）/ `'rotated-after-9074'`（9074 换号路径） |
...
>
> ⚠️ **T9 第三次修正（2026-09-20）的观测成立、归因最终也被证实**：T9 原结论
> 「status / claim **都不校验设备号形态**」（16 位十进制号 / `BoundDeviceID` /
> 空串返回逐字节相同）**观测仍然成立** —— 服务端确实不校验设备号的**形态**；
> 由它推出的「不校验形态 ≠ 不校验**设备**」**最终被 2026-09-23 的单变量矩阵
> 证实**（第 4 次定性一度把它又否掉，但那一次也作废了）：服务端**确实在认设备**，
> 判据是「**这个号有没有进过黑名单**」，而**不是**「它是不是登录时那个」。
> 故签到头用**登录时注册的 16 位号**（`checkin_device_id`）这条**保留**（它是
> 设备头该发的值，缺了回 `9004`），且新增一条：遇 `9074` 时**换一个同形态的
> 全新号重试**（见「积分领取」章节）。
...
| **退避不换号** | `4007` `3004`（软限流）、**`9074`（签到：设备号被拉黑）**、**`3003`（MODEL_FAIL，基础设施类）**、`4000005` `4050`–`4052`（排队） | 排队与基础设施故障都是**全局**状态，换号只会把同一个问题再问一遍并多烧一个账号的额度；`9074` 见「积分领取」章节的定性小节（前四次定性「瞬时频次软限流」「活动级当日名额」「设备身份」「名额/风控」**均已作废**，现行定性是**设备号被拉黑**）。⚠️ 这里说的「不换号」是**chat 侧**的动作；**签到侧**对它另有处置 —— **换设备号重试**（不是换账号），见「积分领取」 |
...
错误文案追加一句「（该模型已不在 Trae CN 可用目录中，请在 Hub 的模型列表里重选）」。
...
**在哪选**：Account Hub → 该 provider 面板 → 点「模型列表」。有多个窗口可选的那些模型行
...
[模型列表的回填机制与过滤](#模型列表的回填机制与过滤)），那条路径不带窗口字段 ⇒
...
- 设备头是 **claim 的硬要求**，缺失时服务端回 `code:9004`。
  ...
  **不是**登录 exchange 返回的 `BoundDeviceID`。
  ⚠️ **归因已于 2026-09-23 定案（第五次）**：T9 的「设备**号形态**不被校验」观测
  **成立**，且它推出的「服务端在校验**设备**」这一步**最终是对的** —— 判据是
  「**这个号有没有进过黑名单**」，**不是**「它是不是登录时那个」。
  ⚠️ 第 4 次定性（本段曾写的「服务端名额/风控、与设备无关」）**已作废**：
  同一账号同一 token、11 秒后换全新号首次 claim 即 `code:0`。
  设备头照发（它是 `9004` 的判据）；遇 `9074` 则**换一个同形态的新号重试一次**
  （见「积分领取」章节）。
...
- `req_source:1` 照抄**唯一次实测成功**的组合。✅ **T1 已校准**：带与不带服务端返回
  **逐字节相同**，它不是 9004 的成因；保留它只因为成本是零。✅ 它同样**不是**
  `9074` 的条件（单变量矩阵已排除）。
...
| 位置 | 取值 | 用途 |
|---|---|---|
| **登录 URL 的 `device_id`**（`generateTraeCnDeviceId`） | 登录时现场生成的 **16 位纯十进制** | 登录握手形态要求；落盘为 `checkin_device_id`，作为签到头 `x-device-id` |
| **exchange 返回的 `BoundDeviceID`**（凭据的 `device_id`） | 14 位字母数字（如 `wl2k1e2endpp32`） | 服务端绑定标识；chat 侧头集用它，**签到侧不用** |

本文档前面「`device_id` 是 16 位纯十进制」讲的是**登录 URL 那个**。

> ⚠️ **`checkin_device_id` 现在有第二种来历**（2026-09-23）：签到遇 `9074` 时
> 它会被换成**轮换生成的新 16 位号**，并由 `device_id_source` 的
> `'rotated-after-9074'` 标记出来 —— 故「凭据里的这个号 = 登录时那个」**不再是
> 恒真命题**，读凭据时要看标记。两种来历的形状相同（同一个生成器），差别只在
> 这个号有没有进过服务端的黑名单。

**四次归因错误的更正记录**（每一轮都记下来，避免再回到已被推翻的结论）：

1. 早先把「形态不符」的后果记成「会触发 9074 风控」—— **错**，`9074` 与**形态**
   无关；
2. 接着把 9074 定性为「活动级当日名额/账号风控」—— **也错**（当时见官方同期
   签成功就否掉了名额，属误读）；
3. 再定性为「**设备身份**：我们把登录注册的设备号丢了」—— **归因方向对了**，
   但判据说错了：服务端认的不是「登录时那个号」，而是「**没进过黑名单的号**」；
4. 随后据此否掉第 3 次、定性为「**名额/风控类拒绝，与设备号取值无关**」——
   **也错**（换全新号 11 秒后即成功，与设备取值直接相关）。
   **现行定性（第 5 次）：设备号被服务端拉黑** —— 该号在**未产出奖励**的 claim
   中出现过。处置见「积分领取」章节：**换号重试一次 + 写回新号**。

#### 与真机的差异（2026-09-20 修正后）

| 项 | 真机 | 本实现 | 处置 |
|---|---|---|---|
| `x-device-id` | `guaranteedDeviceId`（AHA 16 位号，与登录 URL 同源） | **登录时注册的 16 位号**；遇 `9074` 后换成**同形态的新号**（旧凭据降级为 `BoundDeviceID`） | chat 语义已对齐；签到侧新增换号重试（**这是 `9074` 的解药**，`9004` 的判据始终是设备头本身） |
...
⚠️ **「身份保真」类修复与 `9074` 的关系要说清**：改 `x-os-version` /
`x-app-version` 是为了让出站身份与真实客户端一致（消除「服务端按身份归因时看到
一个不存在的客户端形态」这类隐患），它们**不是** `9074` 的解药；`9074` 的解药是
**换一个没进过黑名单的设备号**（本插件已自动执行）。两者是**独立**的两件事。

**签到领取在 claim 段只对瞬时软限流做有界重试**（2026-09-23 收窄）：命中
`4007` / `3004` 时按 1s → 3s 退避重试 2 次（累计 4s），status（读）段**不重试**，
`9004` / `1001` / `9095` **绝不重试**；重试耗尽后按**最后一次**的 code /
message / logid 返回。⚠️ **`9074` 已移出该表**：它的重试是**换设备号那一次**
（硬编码 1 次、不退避），仍是 9074 则归 `unavailable`、不写今日状态。
详见「积分领取」章节。
...
归一后记进 `contextTiers`（升序去重、**≥2 档才记**），Account Hub 的「模型列表」据此
...
| 「登录账号」 | ✓ **浏览器设备流**（登录弹窗 + 轮询） | ✓ 同一套实现，只是授权页指向 `qoder.cn` |
| 积分行 / 「刷新积分」 | ✓ 三池之和的**单数字**与资源包明细 | ✓ 同口径（CN 实测两池，容缺解析天然兼容） |
| 「一键领取积分」 | ✓ 同协议（**两区均已接线**、按 region 分派） | ✓ **`sash/api/v1/me/campaigns` 两步**（见「积分领取」） |
| 卡片操作（刷新 / 删除 / 启停 / 签到） | ✓ 「刷新」= **重打一次 exchange** | ✓ 同上，打的是 **CN 的** exchange |
| 「模型列表」（模型开关） | ✓ 作用于 **`qoder` 这个键** | ✓ 作用于 **`qoder-cn` 这个键**（两区模型池不同，黑名单必须分开） |
