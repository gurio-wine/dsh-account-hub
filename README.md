...
`maxContextWindow` / `contextBudget`）。判据只有一条：**档位数据来自适配器目录
（`contextTiers`），与用户的显示开关无关** —— 关掉一个模型只是让它从对话框选择器
里消失，它在目录里的 dev / Max 档一字未变；而 `model.setContextBudget` 的校验读的
...
> **不追加**。`trae-cn-work` 共用同一批账号与同一个提示函数，但它的 `4008` **未实测**，
> 故只说「已用尽或受限」，不复述 IDE 路径的结论。
...
agent 池目录，见下文「上下文窗口：dev / Max 档位可选」。
...
dev / Max / 当前预算三个字段，档位 radio 照常渲染、照常可改。早前 `model.list` 只在
未被过滤的那批模型上加窗口字段，而关闭的模型是**由回填侧补回列表**的（见
