 ...
 * 2. **字段类型/名字与直觉不符**（两个真实缺陷叠加）：`campaignId` 是**数字**、
 *    可领积分数是 **`benefitAmount`**、不可领取活动的 `status` 是 **`null`**。
 * 3. **账户类型门控**：Token 计费账户必须判为 `inactive` 而不是 `failed`
 *    —— 前者是正常业务状态，后者会让用户去排查并不存在的故障。
 * 4. **幂等预检**：本协议没有幂等键、也没有「今天已签到」业务码，
 *    唯一的保护就是 `claimable` / `status` 预检；漏了会重复发领取请求。
 *
 * ⚠️ 用例必须喂**真实响应形状**：早期版本喂的是编造的 `campaignId: 'c-1'`
 * 与 `amount: 1000`，因此完全没抓到上面第 2 类缺陷。
 */
...
describe('claimCodeArtsDailyCheckin（四步领取）', () => {
  ...
  it('CONFIRMED / CONSUMED 同样判为 already-claimed（三种已领取态）', async () => {
    // 幂等保护只有这一处：`status` ∈ {CLAIMED, CONFIRMED, CONSUMED} 即已领。
    // 漏掉后两种会让「已确认」「已核销」的活动被当成可领，重复发领取请求。
    for (const status of ['CONFIRMED', 'CONSUMED']) {
      const { fetcher, calls } = claimFetcher({
        items: [{ campaignId: 'c-1', type: 'USER_LOGIN', claimable: false, status }],
      })
      const outcome = await claimCodeArtsDailyCheckin(CREDENTIAL, fetcher)
      expect(outcome.kind, status).toBe('already-claimed')
      expect(calls.some((call) => call.url.includes('/v1/ops/claim')), status).toBe(false)
    }
  })
  ...
  it('不可领取且 status 为 null 判为 inactive（不是 already-claimed）', async () => {
    // 实测：不可领取的活动 `status` 可能是 **null**。null 不是「已领取」，
    // 判成 already-claimed 会让用户以为今天领过了而不再尝试。
    const { fetcher } = claimFetcher({
      items: [{ campaignId: 2, type: 'USER_LOGIN', claimable: false, status: null }],
    })
    const outcome = await claimCodeArtsDailyCheckin(CREDENTIAL, fetcher)
    expect(outcome.kind).toBe('inactive')
  })
  ...
  it('id 缺失（undefined）时同样不发 confirm', async () => {
    // 判据是「`id` 既不是 null 也不是 undefined 才确认」：服务端不下发该字段
    // 与显式下发 null 是同一种语义（本次领取无需确认）。
    const { fetcher, calls } = claimFetcher({ claimBody: { code: 0, data: { credit: 1000 } } })
    const outcome = await claimCodeArtsDailyCheckin(CREDENTIAL, fetcher)
    expect(outcome.kind).toBe('claimed')
    expect(calls.some((call) => call.url.includes('/v1/ops/confirm'))).toBe(false)
  })

  it('confirm 请求带 campaignId（不带 channel）', async () => {
    // IDE 的 confirm 只回传 campaignId；多发 channel 属于自创字段。
    const { fetcher, calls } = claimFetcher({ claimBody: { code: 0, data: { id: 'benefit-1' } } })
    await claimCodeArtsDailyCheckin(CREDENTIAL, fetcher)
    const confirmCall = calls.find((call) => call.url.includes('/v1/ops/confirm'))
    expect(confirmCall?.init?.method).toBe('POST')
    expect(JSON.parse(String(confirmCall?.init?.body))).toEqual({ campaignId: 'c-1' })
  })
  ...
  it('领取流程只读凭据，不触发任何续期（refresh_token 一次性轮换）', async () => {
    // ⚠️ CodeArts 的 `refresh_token` 用一次即作废（`STS5.1806 the refresh token
    // has been used`）。积分模块必须**只用 AK/SK/security_token 发签名请求**，
    // 一次都不许碰续期链路 —— 否则会把用户的凭据提前消耗掉。
    //
    // 判据：整条流程的每一个请求 URL 都落在 snap-access 域内，且凭据对象
    // （含 refresh_token）在调用前后**逐字段不变**。
    const credential: CodeArtsCredential = {
      ...CREDENTIAL,
      refresh_token: 'RT-ONE-SHOT',
      code_verifier: 'VERIFIER',
    }
    const snapshot = JSON.stringify(credential)
    const { fetcher, calls } = claimFetcher({})
    await claimCodeArtsDailyCheckin(credential, fetcher)
    expect(JSON.stringify(credential), '凭据被就地改写').toBe(snapshot)
    expect(calls.length).toBeGreaterThan(0)
    for (const call of calls) {
      expect(call.url.startsWith(`${CODEARTS_SNAP_ENGINE_URL}/`), call.url).toBe(true)
    }
    // 也不得把 refresh_token / code_verifier 发进任何请求体
    for (const call of calls) {
      const body = call.init?.body
      if (typeof body !== 'string') continue
      expect(body).not.toContain('RT-ONE-SHOT')
