 ...
 * 账号拖拽排序的纯逻辑测试。
 *
 * 组件无法在本仓库单测里渲染（react 不在依赖里），但拖拽最容易出错的部分
 * 正是「落点算在哪」—— 把它抽成纯函数后就能用真实断言覆盖，
 * 而不是靠源码级字符串匹配去间接验证。
 *
 * 渲染层（事件接线 / 乐观更新 / 回滚）由 `account-order-panel.spec.ts` 负责，
 * 两者分工：本文件证明**算得对**，那份证明**接得上**。
 */
describe('orderAfterDrop（拖拽落点计算）', () => {
  ...
  it('结果始终是原集合的一个排列（元素不增不减）', () => {
    ...
    // 这条对 RPC 尤其重要：`account.reorder` 要求 orderedIds 是**排列**，
    // 少一个 id 会被宿主判 bad-request 且不写盘。
