 ...
 * | provider    | 认证方式                    | 签到流程                              |
 * |-------------|-----------------------------|---------------------------------------|
 * | `buddy-cn`  | Bearer + 腾讯系归属头        | 两步（状态 + 领取）                    |
 * | `lobsterai` | 纯 Bearer，无签名            | 三步（slot + context + check_in）      |
 * | `trae-cn`   | Cloud-IDE-JWT + 设备头       | 两步（status + claim）                 |
 * | `codearts`  | **SDK-HMAC-SHA256 签名**     | **四步（账户类型 + 活动 + 领取 + 确认）** |
 *
 * 华为云这套与前三者没有一处共用，故独立成文件；但**复用 `credits.ts` 的
 * `ClaimOutcome` / `CreditBalance` / `CreditPackage` 类型**，使
 * `computeClaimSummary` 与 Account Hub 的结果摘要 UI、`CreditBalanceRow`
 * 组件一行都不用改。
 *
 ...
 * ## 幂等
 *
 * 与 Buddy CN 靠服务端业务码、LobsterAI 靠客户端 `idempotencyKey` 都不同：
 ...
 * ## 凭据：本模块只读，绝不刷新
 *
 * ⚠️ CodeArts 的 `refresh_token` 是**一次性轮换**的（用一次即作废，服务端回
 * `STS5.1806 the refresh token has been used`）。故本模块只用凭据里的
 * AK/SK/security_token 发签名请求，**不做任何续期**：刷新归 `src/refresh.ts`
 * 的调度器，且任何刷新都必须**立刻回写**新凭据。E2E 凭据读取同理 —— 只读，
 * 不刷新。
 */
...
export async function claimCodeArtsDailyCheckin(
  ...
  return {
    ...
    // CodeArts 的活动不下发连续签到天数概念（那是 Buddy 的机制）。
    streakDays: 0,
    isStreakDay: false,
  }
}
