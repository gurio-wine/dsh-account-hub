 ...
 * **所有 provider 都在本地零点重置**。该假设对 Qoder **两区**都不成立 ——
 * 实测它们的签到窗口都在**本地 10:00** 才翻页（见 {@link CHECKIN_RESET_HOURS}）。
 * 用日数表达时，这些 provider 在 00:00–10:00
 ...
 * - 无记录 → 可签（从未签过）
 *
 * 时区口径仍然是**本地时区**（用户在中国，Qoder 两区的 10 点就是本地 10 点），
 * 与旧模型一致。
 *
 * ## 本模块是「周期」这件事的唯一真相源
 *
 ...
 * | provider | 重置钟点 | 依据 |
 * |---|---|---|
 * | `qoder` / `qoder-cn` | **10** | 官方公告「每日 10:00 (UTC+8) 刷新」+ 两区真机响应（见下节） |
 ...
 *
 * ## `qoder`（国际版）与 `qoder-cn` 同为 10 点：**两区各写一行、值相同**
 *
 * 两区是**同协议、两套 host、两批账号**（见 `src/qoder-product.ts`）。早期这里
 * 给国际版配 0 点，理由是「只观测到 CN 是 10 点，国际版没有证据」—— 该理由
 * 已被推翻，两区**各有独立证据**指向同一个窗口：
 *
 * 1. **官方公告**（`docs.qoder.com/events/100credits`）：「2026-09-18 10:00 (UTC+8)
 *    起，每天可领 100 Credits」，窗口口径为「每天 10:00 开新窗口、次日 10:00 前
 *    关闭」（见 `docs/qoder-integration-research.md` 的 D2 节）。公告面向的是
 *    **国际版个人用户**，CN 是同一活动系统的另一区。
 * 2. **两区各自的真机响应**（本仓库留存的证据文件
 *    `tests/e2e/qoder-minimal-headers-claim-evidence.json`）：国际版
 *    `openapi.qoder.sh` 与 CN `openapi.qoder.com.cn` 的
 *    `/sash/api/v1/me/campaigns` 都下发 `CLAIM_BENEFIT` 活动，其中
 *    `startAt: 1790128800` = **2026-09-23 10:00 (UTC+8)**、`endAt: 1790215140`
 *    = 次日 09:59 (UTC+8)，且 `description` 两区同为
 *    「Daily reset: 10:00 (UTC+8)」/「每日 10:00（UTC+8）刷新」。
 *
 * 故**两行都写 10**。仍**刻意保留两行**（不合并成一条正则或 `startsWith`）：
 * 「活动窗口几点翻页」是**活动系统**的属性、不是协议属性，两区将来任一区改窗口
 * 都必须能只改自己那一行 —— 合并会把「共享协议」误当成「共享窗口」。若某区确实
 * 观察到不同钟点，改**该区那一行**，并把观测写进这里。
 *
 * ## 缺省即 0
 *
...
export const CHECKIN_RESET_HOURS: Readonly<Record<string, number>> = {
  // Qoder 两区**各占一行、值都是 10**：同协议但窗口是两套活动系统的事，
  // 保留两行是为了「任一区改窗口只动自己那一行」（理由见上节，勿合并）。
  'qoder-cn': 10,
  qoder: 10,
 ...
 * 对 Qoder 两区（10 点）则向**后**取到 `d+1` 天的 10 点：旧模型在这里本来就
