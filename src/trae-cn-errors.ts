 ...
 * - `9074`（「当前参与用户太多，请稍后再试」）：⚠️ **定性已于 2026-09-20
 *   **第三次**修正**（前两次均作废）。第 1 次「瞬时频次软限流」被三日取证推翻
 *   （同账号 09-17/18/19 报错 15 / 187 / 250 次、**8 秒退避重放仍 9074**）；
 *   第 2 次「活动级当日容量/名额限制或账号侧风控」也被推翻（官方客户端同期可签
 *   成功）。**现行定性：设备身份** —— 服务端按 `x-device-id` 做设备维度签到
 *   记账，我们发的 `BoundDeviceID` 不被活动系统认可。决定性证据是 status 端点
 *   单变量 A/B：全套头不变、**仅**换 `x-device-id` 即让 `did_checked_in` 由
 *   `false` 翻转为 `true`。仍留在本表（动作仍是「退避、不换号」：设备身份是
 *   账号自己的属性，换号只会多烧一个账号的往返），但**它不记冷却徽章**
 *   （见 {@link recordsTraeCnCooldown}），且签到侧的引导文案指向**重新登录**
 *   （`src/trae-cn-credits.ts` 的 `describeFailureCode`）。
 *   签到侧的有界重试另有一张更窄的清单（`TRAE_CN_CLAIM_RETRY_CODES`）。
