...
⚠️ **与上下文档位红线的关系**：见上一条 —— 那条红线**只管档位**，`max_tokens` 是正交关注点（输出上限），既有逐字节用例仍全绿。
...
- **Trae CN**：`POST /trae/api/v2/pay/web_user_ent_usage`，body `{"require_usage":true}`。
  ...
  - ✅ **T7 已按真机校准**：该端点响应**没有 `code` 信封**（顶层是 `is_credits_billing` / `usage_summary` / `user_entitlement_pack_list`），沿用 code 信封会让余额**恒失败**；礼包数组在**根层**，额度嵌在 `entitlement_base_info.product_extra.package_extra.quota.credits_limit`（回退 `entitlement_base_info.quota`）减 `usage.credits_amount`（可为 `{}`，按 0 计）。候选表 + 指纹扫描 + 三级回退**保留作兜底**，主路径是嵌套口径。✅ **T8 已按真机定案（2026-09-20）**：`claim` 的完整响应就是 `{"code":0,"message":"success"}`、**没有积分字段** ⇒ 旧候选表 `TRAE_CN_CLAIM_CREDIT_FIELDS` 注定落空（表现为「领取成功 **+0** 积分」）；积分数只在 **status 的 `credits`**（真机 150，与余额里「签到奖励」包 `credits_limit:150` 吻合），故 `code===0` 后**补查一次 status** 取它（复用 `fetchTraeCnCheckinStatus`，不新写网络代码），候选表与 `readClaimedCredit` **已整体删除**；**补查失败不改变 `claimed` 语义**（credit 落 0 + onDebug 记原因），补查**不重试**。
