...
积分能力取值矩阵（credits-capabilities.js 为唯一真相源）与「两区取值不同、理由也不同」的完整判据见 docs/agents/credits.md。Qoder 专属要点：qoder 无签到活动（false，每日 100 Credits 只能在桌面 App 领）；qoder-cn 端点已真机验收（true）。