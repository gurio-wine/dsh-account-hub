 ...
 * 源码级字符串匹配。
 *
 * 顺序不是 UI 装饰：它直接决定 `AccountPool.getAvailableAccount` 的候选优先级
 * （见 `src/account-pool.ts` 的 `reorderAccounts` 与 README「账号顺序 = 选号优先级」），
 * 因此这里算出来的数组会被原样送进 `account.reorder` 的 `orderedIds` ——
 * **必须是原集合的一个排列**，否则宿主回 `bad-request` 且拒绝改动数据。
 */
